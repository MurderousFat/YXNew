...
@Controller("BiUnionAction")
@Scope(value="prototype")
public class BiUnionAction extends BaseAction {
    
    @Autowired
    private BiUnionService biUnionService;
    

    public String listPage() throws IOException{
        
        PageBean pageBean = biUnionService.listPage(this.getPageParam(),this.getRequestDto());
    ...
    public String list() throws IOException{
        ...
        List<BiUnionDto> list = biUnionService.listBy(this.getRequestDto());
    ...
    private String save() throws IOException{
        ...
        BiUnionDto biUnionDto = new BiUnionDto();
        DataObjectUtils.mapToObject(biUnionDto, this.getRequestDto());
        try{
            long result = biUnionService.saveTree(biUnionDto);
            if(result<1){
    ...
    public String remove() throws IOException{
        ...
        try{
            biUnionService.deleteByIds(TenzeUtils.str2IdList(key));
                setSuccessTipMsg();
                return NULL_SUCCESS;
        }catch(Exception e){
            e.printStackTrace();
