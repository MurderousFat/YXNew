...
public class TypeCaseHelper {
			...
			if ("Timestamp".equals(type) || "java.sql.Timestamp".equals(type)) {
				...
				if (format == null || format.length() == 0)
					try {
						...
						try {
							DateFormat df = DateFormat.getDateTimeInstance(3,
									3, locale);
							java.util.Date fieldDate = df.parse(str);
	...
	public static Date convert2DateStart(String obj)
			throws TypeCastException {
		if (obj.length() > 10) {
			obj = obj.substring(0, 10);
		}
		obj += " 00:00:00";
		return (Date) convert(obj, "Date", "yyyy-MM-dd HH:mm:ss");
	}
	public static Date convert2DateEnd(String obj)
			throws TypeCastException {
		if (obj.length() > 10) {
			obj = obj.substring(0, 10);
		}
